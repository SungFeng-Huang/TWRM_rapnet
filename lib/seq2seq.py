...
def embedding_attention_seq2seq2seq(encoder_inputs,
                                    decoder_inputs,
                                    cell,
                                    num_encoder_symbols,
                                    num_decoder_symbols,
  ...
  """
  with variable_scope.variable_scope(
      "embedding_attention_seq2seq2seq", dtype=dtype) as wrap_scope:
    with variable_scope.variable_scope(
        "embedding_attention_seq2seq", dtype=dtype) as scope:
      dtype = scope.dtype
      # Encoder.
      encoder_cell = copy.deepcopy(cell)
      encoder_cell = core_rnn_cell.EmbeddingWrapper(
      ...
      trans_cell = copy.deepcopy(cell)
      trans_outputs, trans_state = rnn_decoder(
          encoder_outputs, encoder_state, trans_cell, loop_func, "trans_rnn_decoder")
   
      # First calculate a concatenation of encoder outputs to put attention on.
      top_states = [
          array_ops.reshape(e, [-1, 1, cell.output_size]) for e in trans_outputs
      ]
      attention_states = array_ops.concat(top_states, 1)
      ...
      if isinstance(feed_previous, bool):
        decoder_outputs, decoder_state = embedding_attention_decoder(
            decoder_inputs,
            trans_state,
            attention_states,
            decoder_cell,
            ...
            feed_previous=feed_previous,
            initial_state_attention=initial_state_attention)
        # return outputs, state, trans_state
        ...
        outputs_len = len(decoder_inputs)  # Outputs length same as decoder inputs.
        state_list = outputs_and_state[outputs_len:]
        decoder_state = state_list[0]
        if nest.is_sequence(trans_state):
          state = nest.pack_sequence_as(
              structure=trans_state, flat_sequence=state_list)
        decoder_outputs = outputs_and_state[:outputs_len]
        # return outputs_and_state[:outputs_len], state, trans_state
        #
    wrap_scope.reuse_variables()
    ae_outputs, ae_state, ae_encoder_state = embedding_attention_seq2seq(
        decoder_inputs,
        decoder_inputs,
        cell,
        num_encoder_symbols,
    ...
    return {
        ...
        'enc': encoder_state,
        'trans': trans_state,
        'ae_enc': ae_encoder_state
      }

def embedding_attention_seq2seq2seq_gan(encoder_inputs,
                                    decoder_inputs,
                                    cell,
                                    num_encoder_symbols,
                                    num_decoder_symbols,
  ...
  """
  with variable_scope.variable_scope(
      scope or "embedding_attention_seq2seq2seq_gan", dtype=dtype) as gan_scope:
    outputs, state, encoder_state = embedding_attention_seq2seq2seq(
        encoder_inputs,
        decoder_inputs,
        cell,
        num_encoder_symbols,
    ...
    with variable_scope.variable_scope(
      ...
      # disc_trans = nn_ops.xw_plus_b(nn_ops.leaky_relu(
          # nn_ops.xw_plus_b(encoder_state['trans'], disc_w1, disc_b1)), disc_w2, disc_b2)
      
      # Discriminator.
      # discriminator_cell = copy.deepcopy(cell)
      # discriminator_cell = core_rnn_cell.EmbeddingWrapper(
          # discriminator_cell,
          # embedding_classes=num_decoder_symbols,
          # embedding_size=embedding_size)
      # d_dec_out_outputs, d_dec_out_state = core_rnn.static_rnn(
          # discriminator_cell, encoder_inputs + decoder_outputs, dtype=dtype)
      # d_dec_inp_outputs, d_dec_inp_state = core_rnn.static_rnn(
          # discriminator_cell, encoder_inputs + decoder_inputs, dtype=dtype)

    return outputs, {
        'dec': state['dec'],
        'ae_dec': state['ae_dec'],
        'enc': encoder_state['enc'],
        'trans': encoder_state['trans'],
        'ae_enc': encoder_state['ae_enc']
      }, {
        'true': disc_true,
        'false': disc_false
      }
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
        ...
        outputs.append(bucket_outputs['dec'])
